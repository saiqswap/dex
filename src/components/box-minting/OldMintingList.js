...
const RoundDetail = ({ product, library, _handleSelectRound }) => {
  ...
  const [selectedProduct, setSelectedProduct] = useState(["ANGEL"]);
  ...
  useEffect(() => {
    const sortSelectedProduct = _handleSort(selectedProduct);

    const check = fakeCombo.filter((c) => {
      const { items } = c;
      return sortSelectedProduct.toString() === _handleSort(items).toString();
    });

    if (check[0]) {
      setSelectedItem(check[0]);
    } else {
      console.log(selectedProduct.length);
      if (selectedProduct.length === 0) {
        setSelectedItem(product.items[0]);
      } else {
        const tempSelectItem = product.items.filter(
          (item) => item.boxType === selectedProduct[0]
        );
        setSelectedItem(tempSelectItem[0]);
      }
    }
  }, [selectedProduct]);

  let note = "Please choose 3 type of box to buy combo";
  if (selectedProduct.length > 1) {
    note = "Please choose another type of box to buy combo";
  }
  if (selectedProduct.length > 2 && selectedItem.boxType === "COMBO") {
    note = "you are choosing to buy combo";
  }
    ...
    <>
      ...
      >
        {product.items.map((item, j) => {
          ...
          const check = selectedProduct.includes(item.boxType);
          return (
            <BoxItem
              key={j}
              sx={{
                border: `1px solid ${
                  check ? information.color : "var(--main-color)"
                }`,
              }}
              p={1}
              onClick={() => _handleSelectItem(item.boxType)}
            >
              <img
        ...
        <>
          <CustomStack>
            ...
            <Typography color="greenyellow">
              -{(selectedItem.unitPrice * 10) / 100}{" "}
              {selectedItem.paymentCurrency} (10%)
            </Typography>
          </CustomStack>
          <CustomStack>
            <FieldLabel>{"Total"}:</FieldLabel>
            <Typography>
              {selectedItem.unitPrice - (selectedItem.unitPrice * 10) / 100}{" "}
      ...
      <Stack>
        ...
        >
          {/* {library[status]} */}
          Buy {selectedItem.boxType === "COMBO" && "Combo"}
        </CustomButton>
      </Stack>
    </>
  );
